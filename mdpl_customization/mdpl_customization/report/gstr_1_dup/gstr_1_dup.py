...
class Gstr1Report(object):
    ...
    def run(self):
        ...
        self.get_data()
        # frappe.log_error("data",self.data)
        # frappe.log_error("get_columns",self.columns)
            ...
            for inv, items_based_on_rate in self.items_based_on_tax_rate.items():
                ...
                for rate, items in items_based_on_rate.items():
                    ...
                    if self.filters.get("type_of_business") == "B2C Small":
                        ...
                        frappe.log_error("default_key",default_key)
    ...
    def is_b2cl_cdn(self, invoice):
        ...
        frappe.log_error("test",grand_total > B2C_LIMIT)
    ...
    def get_row_data_for_invoice(self, invoice, invoice_details, tax_rate, items):
        ...
        account_head=frappe.db.sql(f" select account_head,tax_amount from `tabSales Taxes and Charges` where parent = '{row[2]}'",as_dict=1)
        # frappe.log_error("account_head",account_head)
        ...
        for column in self.other_columns:
            if column.get("fieldname") == "cess_amount":
                ...
                # if account_head:
                #     acc_list=[]
                #     for acc in account_head:
                #         if "IGST" in acc["account_head"]:
                #             igst=acc["tax_amount"] 
                #         elif "CGST" in acc["account_head"]:    
                #             cgst=acc["tax_amount"]
                #         elif "SGST" in acc["account_head"]:
                #             sgst=acc["tax_amount"]
                #     row.append(cgst)
                #     row.append(sgst)
                #     row.append(igst)
    ...
    def get_items_based_on_tax_rate(self):
        ...
        frappe.log_error("self.items_based_on_tax_rate 1",self.items_based_on_tax_rate)
    ...
    def get_columns(self):
        ...
        # frappe.log_error("columns",self.columns)
...
class GSTR1DocumentIssuedSummary:
    ...
    def get_document_summary(self):
        ...
        frappe.log_error("doc data 1",data)
        data = self.handle_amended_docs(data)
        
        # frappe.log_error("doc query ")
        frappe.log_error("doc data 2 ",data)
        summarized_data = []
    ...
    def seperate_data_by_naming_series(self, data, nature_of_document):
        ...
        frappe.log_error("slice_indices",slice_indices)
        document_series_list = [
            data[i:j] for i, j in zip([0] + slice_indices, slice_indices + [None])
        ]
        frappe.log_error("document_series_list",document_series_list)
        ...
        # frappe.log_error("summarized_data",summarized_data)
...
def get_document_issued_summary_json(data):
    ...
    # frappe.log_error("document_lists",document_lists)
